...
__global__ void trace_warp_shuffle_kernel(const T* __restrict__ input, T* __restrict__ output, const size_t diag, const size_t cols, const size_t step) {
    ...
    T sum = (T)0;
    #pragma unroll
    for (size_t i = idx; i < diag; i += step) { // grid-stride loop
        sum += input[i * cols + i];
    }

    T warp_reduce_sum_float = warp_reduce_sum(sum);
    if(tid % 32 == 0){
        smem[tid / 32] = warp_reduce_sum_float;
    }
    __syncthreads();
...
// 方法二：用cg实现的跨块归约的trace计算核函数（无atomicAdd）
...
__global__ void trace_cooperative_reduce_kernel(const T* __restrict__ input, T* __restrict__ partial, // 长度至少 gridDim.x，最终结果写 partial[0]
    ...
    T warp_reduce_sum_float = warp_reduce_sum(sum); // warp 内归约

    if ((tid & 31) == 0) {
        smem[tid >> 5] = warp_reduce_sum_float;
    }
    block.sync();
    ...
    T* __restrict__ O)
{
    ...
    for (int j = 0; j < s_end_block; j += KV_tilesize) {
        ...
        for (int q = 0; q < Qm; ++q) { 
            ...
            if (wid == 0) {
                float v = (lane < num_warps) ? smem_warpbuf[lane] : -INFINITY;
    ...
    if constexpr (std::is_same_v<T, half>) { //is_same_v需要C++17
        ...
        flash_attention_kernel<T, KV_tilesize, Bl, Qm>
            <<<grid_dim, block_dim, smem_bytes,stream>>>(
                ...
                softmax_scale, is_causal, d_o);

    } else if constexpr (std::is_same_v<T, float>) {
        constexpr int Bl = 256;
        constexpr int KV_tilesize = 32;
        constexpr int Qm = 16;
                    ...
                    int query_heads, int kv_heads, int head_dim, bool is_causal)
{
    ...
    CUDA_CHECK(cudaMalloc(&d_q, q_elems * sizeof(T)));
    CUDA_CHECK(cudaMalloc(&d_k, k_elems * sizeof(T)));
    CUDA_CHECK(cudaMalloc(&d_v, v_elems * sizeof(T)));
    CUDA_CHECK(cudaMalloc(&d_o, q_elems * sizeof(T)));

    CUDA_CHECK(cudaMemcpyAsync(d_q, h_q.data(), q_elems * sizeof(T), cudaMemcpyHostToDevice, stream));
    CUDA_CHECK(cudaMemcpyAsync(d_k, h_k.data(), k_elems * sizeof(T), cudaMemcpyHostToDevice, stream));
    CUDA_CHECK(cudaMemcpyAsync(d_v, h_v.data(), v_elems * sizeof(T), cudaMemcpyHostToDevice, stream));
    CUDA_CHECK(cudaMemsetAsync(d_o, 0, q_elems * sizeof(T), stream));
    ...
    // ---------GPU结果拷回CPU---------
    CUDA_CHECK(cudaMemcpyAsync(h_o.data(), d_o, q_elems * sizeof(T), cudaMemcpyDeviceToHost, stream));
